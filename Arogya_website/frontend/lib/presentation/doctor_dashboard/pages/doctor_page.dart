                ...
                Expanded(
                  child: BlocConsumer<DashboardBloc, DashboardState>(
                    listenWhen: (previous, current) =>
                        previous.actionResult.actionToken != current.actionResult.actionToken,
                    listener: (context, state) {
                      final result = state.actionResult;
                      if (result.consultation == null) return;
                      if (result.succeeded) {
                        final isJoinCall =
                            result.consultation!.action == ConsultationActionType.joinCall;
                        ...
                        _openPatientChart(context, result.consultation!.patientId);
                      } else {
                    ...
                    builder: (context, state) {
                      if (state.status == DashboardStatus.loading && state.summary == null) {
                        return const Center(child: CircularProgressIndicator());
                      }

                      if (state.status == DashboardStatus.failure && state.summary == null) {
                        return _ErrorState(
                                ...
                                children: [
                                    ...
                                    children: [
                                      Expanded(
                                        ...
                                        child: MorningOverviewCard(
                                          consultationsToday:
                                              summary?.morningOverview.consultationsToday ?? 0,
                                          capacityPercent:
                                              summary?.morningOverview.capacityPercent ?? 0,
                                        ),
                                      ),
                                      ...
                                      Expanded(
                                        child: StatAlertCard(
                                          ...
                                          badgeText: summary != null && summary.newReportsCount > 0
                                              ? '+${summary.newReportsCount} New'
                                              : null,
                                          badgeColor: const Color(0xFFFBDCD5),
                                          label: 'Pending Reports',
                                          value: '${summary?.pendingReportsCount ?? 0}',
                                          onTap: () => _showComingSoon(context, 'Pending reports list'),
                                        ),
                                      ),
                                      const SizedBox(width: 16),
                                      Expanded(
                                        child: StatAlertCard(
                                          ...
                                          value:
                                              '${summary?.criticalAlertsCount ?? 0}'.padLeft(2, '0'),
                                          onTap: () =>
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      TextButton.icon(
                                        onPressed: () => _showComingSoon(context, 'Calendar view'),
                                  ...
                                  if (state.consultations.isEmpty)
                                    const Padding(
                                      ...
                                      child: Text('No consultations scheduled for today.',
                                          style:
                                    ...
                                    for (final c in state.consultations)
                                      ConsultationCard(
                                        data: c,
                                        isActionLoading:
                                            state.actionInProgressConsultationId == c.id,
                                        onAction: () => context
                                            .read<DashboardBloc>()
                                            .add(DashboardConsultationActionPressed(c)),
                                        onOpenFile: () => _openPatientChart(context, c.patientId),
                                      ),
                                ],
                              );
                              ...
                              final rightRail = Column(
                                ...
                                children: [
                                  RecentActivityCard(
                                    ...
                                    onViewFullLog: () => _showComingSoon(context, 'Full activity log'),
                                  ),
                                  const SizedBox(height: 20),
                                  WeeklySummaryCard(
                                    efficiencyDeltaPercent:
                                        summary?.weeklySummary.efficiencyDeltaPercent ?? 0,
                                    progress: summary?.weeklySummary.progress ?? 0,
                                  ),
                                ],
                              );
